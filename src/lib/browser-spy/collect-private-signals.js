...
//  const storage = Math.round((await navigator.storage.estimate()).quota / 100000000); // incognito measures are unreliable and overtime change needs to be accounted for (likely means automatically updating it as well)
  ...
  const auth_string = `screen_resolution=${screen_resolution}&cores=${cores}&gpu=${gpu}&language=${language}&useragent=${useragent}&storage=${storage}&timezone=${timezone}&webgl_hash=${webgl_hash}&canvas_hash=${canvas_hash}&spoofed_canvas=${spoofed_canvas}&ip_data=${ip_data?.city},${ip_data?.asn}&time=${time}`;
