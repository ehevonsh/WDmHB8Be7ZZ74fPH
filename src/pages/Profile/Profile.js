...
import { getPlatformUser, updatePlatformUser } from "../../lib/strapi/platform-user";
  ...
  return (
    <main className="gridBox my-6">
      <section className="rounded">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <button 
              ...
              disabled={isUpdating}
              className={`text-white font-semibold px-4 py-2 rounded shadow-md active:translate-y-px w-full sm:w-auto ${isUpdating ? 'bg-gray-400 cursor-not-allowed' : 'bg-purple'}`}
            >
        ...
        {updateMessage && (
          <div className={`mt-4 p-2 rounded text-sm ${updateMessage.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {updateMessage.text}
          </div>
        )}
